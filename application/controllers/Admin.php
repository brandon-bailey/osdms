...
class Admin extends CI_Controller {
	...
	public function saveSettings() {
		if ($this->session->admin) {
			...
			$this->settings_model->save($file, $data);
		} else {
			$this->session->set_flashdata('error', 'You do not have permission to edit the admin settings.');
			redirect('home');
		}
	...
	public function fileTypes() {
		if ($this->session->admin) {
			...
			$data = array(
				'pageTitle' => 'Edit File Settings',
				'file' => 'upload',
			);

			$this->load->view('templates/header', $data);
			$this->load->view('admin/edit_settings_view', array('settings' => $this->settings->loadSettings('upload')));
			$this->load->view('templates/footer');
