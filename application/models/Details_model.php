...
class Details_Model extends CI_Model
{
    ...
    public function getFileDetails()
    {
        ...
        if (strchr($this->requestId, '_')) {
            ...
            $fileSize = $this->globalFunctions->displayFilesize($this->config->item('revisionDir') .
             $this->requestId . '/' . $this->requestId . '_' . $revisionId);
        }
...
//get the number of revisions.
        ...
        foreach ($historyResult->result() as $row) {
            ...
            if (is_file($this->config->item('revisionDir') . $this->requestId
                . '/' . $this->requestId . "_$revision" . '.' . $fileExt)) {
                $revision = '<a href="details/' . $this->requestId . "_$revision" .
                 '"><div class="revision">' . ($revision + 1) . '</div></a>';
            }
            $fileHistory[] = array(
        ...
        if ($status == 0 or ($status == -1 && $fileDataObj->isOwner($this->session->id))) {
            ...
            if ($userPermissionObj->getAuthority($this->requestId, $fileDataObj) >= $this->config->item('WRITE_RIGHT')
             && !isset($revisionId) && !$archived) {
            ...
            if ($userPermissionObj->getAuthority($this->requestId, $fileDataObj) >= $this->config->item('ADMIN_RIGHT')
             && !@isset($revisionId) && !$archived) {
