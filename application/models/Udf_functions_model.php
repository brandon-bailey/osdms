...
class Udf_Functions_Model extends CI_Model
{
    ...
    public function udfFunctionsAddUdf($post)
    {
        ...
        if ($query->num_rows() === 0) {
            if ($post['udfType'] == 1 || $post['udfType'] == 2) {
                ...
                if ($this->db->trans_status() === false) {
                    $this->db->trans_rollback();
                    $this->session->set_flashdata('error', 'There was a problem with inserting new 
                        udf field into the udf table.');
                    redirect($_SERVER['HTTP_REFERER']);
                    exit;
                } else {
                    $msg = array(
                ...
                // First we add a new column in the data table
                    ...
                    if ($this->db->trans_status() === false) {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('error', 'There was a problem with creating new 
                    ...
                    if ($this->db->trans_status() === false) {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('error', 'There was a problem with creating 
                            the new primary table in the database.');
                        redirect($_SERVER['HTTP_REFERER']);
                        exit;
                    }
                    ...
                    if ($this->db->trans_status() === false) {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('error', 'There was a problem with creating 
                            the new secondary table in the database.');
                        redirect($_SERVER['HTTP_REFERER']);
                        exit;
                    }
                    $data = array(
                    ...
                    if ($this->db->trans_status() === false) {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('error', 'There was a problem with inserting 
                            the updates into the udf table.');
                        redirect($_SERVER['HTTP_REFERER']);
                        exit;
                    } else {
                            $msg = array(
                ...
                if ($this->db->trans_status() === false) {
                    ...
                    if ($this->db->trans_status() === false) {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('error', 'There was a problem adding 
                            the new data to the UDF table.');
                        redirect($_SERVER['HTTP_REFERER']);
                        exit;
                    } else {
                        $msg = array(
    ...
    {
        $lookupQuery = $this->db->select('table_name, field_type, display_name')
        ->order_by('id')
        ->get('udf');

        foreach ($lookupQuery->result() as $row) {
     ...
     * @param string $where
     * @param string $queryPre
     * @param string $query
     * @param stting $equate
     * @param string $keyword
     * @return array
     */
    public function udfFunctionsSearch($where, $queryPre, $query, $equate, $keyword)
    {

        $lookupQuery = $this->db->select('table_name, field_type')
        ->where('display_name', $where)
        ->get('udf');

        $row = $lookupQuery->result();
